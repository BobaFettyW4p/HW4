 ...
 * If maxlen was initialized to a different value, the program would output that value instead
 * 
 * We can resolve this by utilizing std::ref, which allows us to pass objects by reference instead
...
maxlenftn maxf;
for_each(names.begin(), names.end(), std::ref(maxf));
